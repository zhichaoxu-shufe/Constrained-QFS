...
class ConstrainedHypothesis:
    ...
    def num_met(self) -> int:
        ...
        if not self.clauses:
            return 0
        #print([(int(c.satisfy), str(c)) for c in self.clauses])
    ...
    def advance(self, word_id: int, not_from_init: bool = True) -> 'ConstrainedHypothesis':
        """
        Updates the constraints object based on advancing on word_id.
            ...
            if temp_pos_state.met_phrases is not None:
                ...
                for phrase in temp_pos_state.met_phrases:
                    for clause in obj.clauses:
                        if not clause.satisfy and phrase in clause.positive and not_from_init:
